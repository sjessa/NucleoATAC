...
class FragmentMixDistribution:
    ...
    # input file and input type (BAM or fragments) are passed to caclulateSizes
    def getFragmentSizes(self, input_file, input_type, chunklist = None):
        self.fragmentsizes = FragmentSizes(self.lower, self.upper)
        self.fragmentsizes.calculateSizes(input_file, input_type, chunks = chunklist)
...
class OccupancyParameters:
    """Class for storing parmeters related to Occupancy determination"""

    def __init__(self, insert_dist, upper, fasta, pwm, sep = 120, min_occ = 0.1, flank = 60,
                 out = None, input_file = None, input_type = None, ci = 0.9, step = 5):
        self.sep = sep
...
class OccChunk(Chunk):
    ...
    def getFragmentMat(self):
        # Updated to take in either BAM or fragments file as input
    ...
    def process(self, params):
        """proces chunk -- calculate occupancy, get coverage, call peaks"""
        self.params = params
        self.getFragmentMat()
        self.makeBiasMat()        
        self.calculateOcc()
